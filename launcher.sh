...
if [ $STOP == "--stop" ]; then
  sudo LD_LIBRARY_PATH=$LINKER_PATH $PWD/build/output/$LOCAL_CURRENT_CHANNEL/$LOCAL_BUILD_TARGET/crosvm stop $PWD/build/output/$LOCAL_CURRENT_CHANNEL/$LOCAL_BUILD_TARGET/crosvm.sock
    ...
    sudo LD_LIBRARY_PATH=$LINKER_PATH --preserve-env=$CURRENT_CHANNEL $PWD/build/output/$LOCAL_CURRENT_CHANNEL/debug/crosvm run --disable-sandbox --rwdisk $PWD/build/output/rootfs.ext4 -s $PWD/build/output/$LOCAL_CURRENT_CHANNEL/$LOCAL_BUILD_TARGET/crosvm.sock -m 10240 --cpus 4 -p "root=/dev/vda" -p $KERNEL_CMD_OPTIONS --host_ip 10.0.0.1 --netmask 255.255.255.0 --mac 9C:B6:D0:E3:96:4D --wayland-sock=$XDG_RUNTIME_DIR/$WAYLAND_DISPLAY --wayland-dmabuf $PWD/build/output/$LOCAL_CURRENT_CHANNEL/vmlinux
fi
